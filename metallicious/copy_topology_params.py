...
def adjust_charge(topol_new, fp_topol, mapping_fp_to_new):
    ...
    for a in mapping_fp_to_new:
        logger.info(
            f"          {topol_new.atoms[mapping_fp_to_new[a]].type:s} "
        ...
        topol_new.atoms[mapping_fp_to_new[a]].charge += fp_topol.atoms[
            a
        ].charge  # topol_fp.atoms[a].charge
...
def adjust_bonds(topol_new, topol_fp, mapping_fp_to_new):
    ...
    for bond_fp in topol_fp.bonds:
        ...
        for bond_new in topol_new.bonds:
            ...
            if (
                bond_fp.atom1.idx in mapping_fp_to_new
                and bond_fp.atom2.idx in mapping_fp_to_new
            ):
                if (
                    bond_new.atom1.idx == mapping_fp_to_new[bond_fp.atom1.idx]
                    and bond_new.atom2.idx == mapping_fp_to_new[bond_fp.atom2.idx]
                ) or (
                    bond_new.atom1.idx == mapping_fp_to_new[bond_fp.atom2.idx]
                    and bond_new.atom2.idx == mapping_fp_to_new[bond_fp.atom1.idx]
                ):
                    if bond_fp.type != bond_new.type:
                        logger.info(
                            f"      [o] Diffrent bond type  {bond_new.type:} -> {bond_fp.type:}"
                        )
                        logger.info(
                        ...
                        type_to_assign = pmd.topologyobjects.BondType(
                            bond_fp.type.k, bond_fp.type.req, list=topol_new.bond_types
                        )
    ...
    for bond_fp in topol_fp.bonds:
        ...
        if not found:
            ...
            topol_new.bonds.append(
                pmd.topologyobjects.Bond(atom1, atom2, type=type_to_assign)
            )
...
def adjust_angles(topol_new, topol_fp, mapping_fp_to_new):
    ...
    for angle_fp in topol_fp.angles:
        found = False
        if (
            angle_fp.atom1.idx in mapping_fp_to_new
            and angle_fp.atom2.idx in mapping_fp_to_new
            and angle_fp.atom3.idx in mapping_fp_to_new
        ):
            for angle_new in topol_new.angles:
                # Check if atoms in mapping (that they are not the additional atoms)
                if (
                    angle_new.atom1.idx == mapping_fp_to_new[angle_fp.atom1.idx]
                    and angle_new.atom2.idx == mapping_fp_to_new[angle_fp.atom2.idx]
                    and angle_new.atom3.idx == mapping_fp_to_new[angle_fp.atom3.idx]
                ) or (
                    angle_new.atom1.idx == mapping_fp_to_new[angle_fp.atom3.idx]
                    and angle_new.atom2.idx == mapping_fp_to_new[angle_fp.atom2.idx]
                    and angle_new.atom3.idx == mapping_fp_to_new[angle_fp.atom1.idx]
                ):
                    if angle_fp.type != angle_new.type:
                        logger.info(
                            f"      [o] Diffrent angle type {angle_new.type:} -> {angle_fp.type:}"
                        )
                        # angle_new.funct = angle_fp.funct
                        # angle_new.type.k = angle_fp.type.k
                        # angle_new.type.theteq = angle_fp.type.theteq

                        type_to_assign = pmd.topologyobjects.AngleType(
                            angle_fp.type.k,
                            angle_fp.type.theteq,
            ...
            logger.info("But it is standard bond so it should be there")
        ...
        if not found:
            ...
            type_to_assign = pmd.topologyobjects.AngleType(
                angle_fp.type.k, angle_fp.type.theteq, list=topol_new.angle_types
            )
            # if type_to_assign not in topol_new.angle_types:
            topol_new.angle_types.append(type_to_assign)

            logger.info(
                f"      [x] New angle:{angle_fp.atom1.name:}-{angle_fp.atom2.name:}-{angle_fp.atom3.name:} "
            ...
            topol_new.angles.append(
                pmd.topologyobjects.Angle(atom1, atom2, atom3, type=type_to_assign)
            )
...
def adjust_dihedrals(topol_new, topol_fp, mapping_fp_to_new):
    ...
    for dihedral_fp in topol_fp.dihedrals:
        ...
        for dihedral_fp_type in dihedral_fp_types:
            if (
                dihedral_fp.atom1.idx in mapping_fp_to_new
                and dihedral_fp.atom2.idx in mapping_fp_to_new
                and dihedral_fp.atom3.idx in mapping_fp_to_new
                and dihedral_fp.atom4.idx in mapping_fp_to_new
            ):
                for dihedral_new in topol_new.dihedrals:
                    ...
                    for dihedral_new_type in dihedral_new_types:
                        if (
                            (
                                dihedral_new.atom1.idx
                                == mapping_fp_to_new[dihedral_fp.atom1.idx]
                                and dihedral_new.atom2.idx
                                == mapping_fp_to_new[dihedral_fp.atom2.idx]
                                and dihedral_new.atom3.idx
                                == mapping_fp_to_new[dihedral_fp.atom3.idx]
                                and dihedral_new.atom4.idx
                                == mapping_fp_to_new[dihedral_fp.atom4.idx]
                            )
                            or (
                                dihedral_new.atom1.idx
                                == mapping_fp_to_new[dihedral_fp.atom4.idx]
                        ...
                        ) and (dihedral_new_type.per == dihedral_fp_type.per):
                            if dihedral_fp_type != dihedral_new_type:
                                ...
                                type_to_assign = pmd.topologyobjects.DihedralType(
                                    phi_k=dihedral_fp_type.phi_k,
                ...
                logger.info(
                    "And you should be worry if it is not the end of fingerprint"
                )
                found = True
            ...
            if not found:
                ...
                topol_new.dihedrals.append(
                    pmd.topologyobjects.Dihedral(
                        atom1, atom2, atom3, atom4, type=type_to_assign
                    )
                )
    ...
    for dihedral_fp in topol_fp.impropers:
        ...
        if (
            ...
            for dihedral_new in topol_new.impropers:
                ...
                if (
                ...
                ) and (dihedral_new.type.per == dihedral_fp.type.per):
                    if dihedral_fp.type != dihedral_new.type:
                        ...
                        type_to_assign = pmd.topologyobjects.ImproperType(
                            psi_k=dihedral_fp.type.psi_k,
...
def adjust_pair_exclusions(topol_new, topol_fp, mapping_fp_to_new):
    ...
    for pair_fp in topol_fp.adjusts:
        ...
        for pair_new in topol_new.adjusts:
            # Check if atoms in mapping (that they are not the additional atoms)
            if (
                pair_fp.atom1.idx in mapping_fp_to_new
                and pair_fp.atom2.idx in mapping_fp_to_new
            ):
                if (
                    pair_new.atom1.idx == mapping_fp_to_new[pair_fp.atom1.idx]
                    and pair_new.atom2.idx == mapping_fp_to_new[pair_fp.atom2.idx]
                ) or (
                    pair_new.atom1.idx == mapping_fp_to_new[pair_fp.atom2.idx]
                    and pair_new.atom2.idx == mapping_fp_to_new[pair_fp.atom1.idx]
                ):
                    found = True
            else:
                found = True
        ...
        if not found:
            ...
            type_to_assign = pmd.topologyobjects.NonbondedExceptionType(
                atom1.rmin + atom2.rmin, 0.5 * (atom1.epsilon + atom2.epsilon)
            )

            topol_new.adjust_types.append(type_to_assign)
            topol_new.adjusts.append(
                pmd.topologyobjects.NonbondedException(atom1, atom2)
            )
...
def add_1_4_metal_pairs(topol_new, metal_index):
    ...
    for idx, node1 in enumerate(all_pairs_1_4):
        for node2 in all_pairs_1_4[idx + 1 :]:
            ...
            if (
                sum(
                    [1 for path in paths if len(set(path)) == 4 and metal_index in path]
                )
                > 0
            ):
    ...
    for pair in pairs_1_4:
        ...
        if (atom1.idx, atom4.idx) not in orginal_pairs or (
            atom4.idx,
            atom1.idx,
        ) not in orginal_pairs:
            type_to_assign = pmd.topologyobjects.NonbondedExceptionType(
                atom1.rmin + atom4.rmin, 0.5 * (atom1.epsilon + atom4.epsilon)
            )
            topol_new.adjust_types.append(type_to_assign)
            topol_new.adjusts.append(
                pmd.topologyobjects.NonbondedException(atom1, atom4)
            )
    return topol_new
...
def copy_bonds(topol_new, bonds, metal_name):
    ...
    for bond in bonds:
        find = [
            a
            for a, bond_o in enumerate(orginal_bonds)
            if (bond_o == bond or bond_o == bond[::-1])
        ]
        if len(find) == 1:
            bond_new = topol_new.bonds[find[0]]

            type_to_assign = pmd.topologyobjects.BondType(
                bonds[bond][1], bonds[bond][0], list=topol_new.bond_types
            )
            ...
            if (
                strip_numbers_from_atom_name(topol_new.atoms[bond[0]].name).title()
                == metal_name.title()
                or strip_numbers_from_atom_name(topol_new.atoms[bond[1]].name).title()
                == metal_name.title()
            ):
                type_to_assign = pmd.topologyobjects.BondType(
                    bonds[bond][1], bonds[bond][0], list=topol_new.bond_types
                ...
                print(
                    "this is wierd bond",
                    topol_new.atoms[bond[0]].name,
                    topol_new.atoms[bond[1]].name,
                    bond,
                    bonds[bond],
                )
    return topol_new
...
def copy_angles(topol_new, angles, metal_name):
    ...
    for angle in angles:
        find = [
            a
            for a, angle_o in enumerate(orginal_angles)
            if (angle_o == angle or angle_o == angle[::-1])
        ]
        if len(find) == 1:
            angle_new = topol_new.angles[find[0]]
            type_to_assign = pmd.topologyobjects.AngleType(
                angles[angle][1], angles[angle][0], list=topol_new.angle_types
            )
            ...
            if (
                strip_numbers_from_atom_name(topol_new.atoms[angle[0]].name).title()
                == metal_name.title()
                or strip_numbers_from_atom_name(topol_new.atoms[angle[1]].name).title()
                == metal_name.title()
                or strip_numbers_from_atom_name(topol_new.atoms[angle[2]].name).title()
                == metal_name.title()
            ):
                type_to_assign = pmd.topologyobjects.AngleType(
                    angles[angle][1], angles[angle][0], list=topol_new.angle_types
...
def copy_dihedrals(topol_new, dihedrals, metal_name):
    ...
    for dihedral in dihedrals:
        find = [
            a
            for a, dihedral_o in enumerate(orginal_dihedrals)
            if (dihedral_o == dihedral or dihedral_o == dihedral[::-1])
        ]
        if len(find) == 1:
            raise Exception(
            ...
            if (
                strip_numbers_from_atom_name(topol_new.atoms[dihedral[0]].name).title()
                == metal_name.title()
                or strip_numbers_from_atom_name(
                    topol_new.atoms[dihedral[1]].name
                ).title()
                == metal_name.title()
                or strip_numbers_from_atom_name(
                    topol_new.atoms[dihedral[2]].name
                ).title()
                == metal_name.title()
                or strip_numbers_from_atom_name(
                    topol_new.atoms[dihedral[3]].name
                ).title()
                == metal_name.title()
            ):
                # we do use periodicy=2 and use defualt values for scee=1.2 and scnb=2 (screening of electrostatics and noboned)
                type_to_assign = pmd.topologyobjects.DihedralType(
                    phi_k=dihedrals[dihedral][1],
                    per=1,
...
def update_pairs(topol):
    ...
    new_pairs = []
    angles = [
        [angle.atom1.idx, angle.atom2.idx, angle.atom3.idx] for angle in topol.angles
    ]
