...
def load_fp_from_file(
    filename_fp_coord, filename_fp_topol, fp_style=None, ignore_truncation_warning=True
):
    """
    ...
    residual_charges = np.array(
        [np.sum([atom.charge for atom in residue]) for residue in topol.residues]
    )
    ...
    if fp_style is None:
        ...
        if fp_style == "dihedral" or fp_style == "dih" or fp_style == "3-bond":
            atoms_bound_to_metal_by_bonded = list(
                nx.generators.ego_graph(G, 0, radius=3).nodes
            )

        elif fp_style == "angle" or fp_style == "ang" or fp_style == "2-bond":
            atoms_bound_to_metal_by_bonded = list(
                nx.generators.ego_graph(G, 0, radius=2).nodes
            )

        elif fp_style == "bond" or fp_style == "1-bond":
            atoms_bound_to_metal_by_bonded = list(
                nx.generators.ego_graph(G, 0, radius=1).nodes
            )
            # atoms_bound_to_metal_by_bonded = list(
        ...
        if std > 0.3 and ignore_truncation_warning == False:
            raise ValueError(
                f"WARNING the truncation results in large overall change of charge of residues: changing truncation scheme might help; std: {std:}"
            )
...
def reduce_site_to_fingerprint(
    ...
    bonds = MDAnalysis.topology.guessers.guess_bonds(
        syst_fingerprint_no_metal.atoms, syst_fingerprint_no_metal.atoms.positions
    )
    ...
    nx.set_node_attributes(
        G_fingerprint,
        {atom.index: atom.name[0] for atom in syst_fingerprint_no_metal.atoms},
        "name",
    )
    G_fingerprint_subs = [
        G_fingerprint.subgraph(a) for a in nx.connected_components(G_fingerprint)
    ]
    logger.info(f"\t\t\t[ ] Mapping fingerprint to metal center: {metal_index:d}")

    G_sub_cages, closest_atoms = find_bound_ligands_nx(
        cage,
        metal_index,
        cutoff=cutoff,
        neighbhor_cutoff=neighbhor_cutoff,
    ...
    if len(G_sub_cages) != len(G_fingerprint_subs) and guessing:
        logger.info(
            f"\t\t\t[!] Not the same number of sites {guessing:}, structure: {number_ligands_bound:d} vs. fingerprint {len(G_fingerprint_subs):d}"
        )
        return False
    ...
    for G_sub_cage in G_sub_cages:
        ...
        for G_idx, G_fingerprint_sub in enumerate(G_fingerprint_subs):
            ...
            if not hasattr(mol1.atoms[0], "element"):
                guessed_elements = MDAnalysis.topology.guessers.guess_types(
                    strip_numbers_from_atom_names(mol1.atoms.names)
                )
            ...
            mol2 = MDAnalysis.Merge(
                syst_fingerprint_no_metal.select_atoms(
                    f"index {' '.join(list(map(str, list(G_fingerprint_sub)))):s}"
                )
            )

            cage_sub_rdkit = mol1.atoms.convert_to(
                "RDKIT", NoImplicit=False
            )  # NoImplicit is use to allow implicit hydrogens (but if they are there it does not remove them!). Important side effect is that it does not bond orders etc. allowing for comparison of structures
        ...
        if guessing:  # if we want to guess site, that might be not fulfilled, and that means it is not the correct site
            if len(largest_common_subgraph) == 0:
                return False
            elif not len(G_fingerprint_subs[finerprint_idx]) == len(
                largest_common_subgraph
            ):
                return False
        else:
            ...
            assert len(G_fingerprint_subs[finerprint_idx]) == len(
                largest_common_subgraph
            )
        ...
        for key in largest_common_subgraph:
            if (
                G_sub_cage_degree[key]
                != G_fingerprint_subs_degree[largest_common_subgraph[key]]
            ):
                end_atoms.append(key)  #

    connected_cut_system = cage.select_atoms(
        f"index {metal_index:}"
    ) + cage.select_atoms(f"index {' '.join(map(str, selected_atoms)):}")
                ...
                if metal_name.title() in name:
                    if metal_charge == int(name.split("_")[1]):
                        if vdw_type is None:
                            ...
                            vdw_type == name.split("_")[2]
                        ):  # if vdw_type is specified we only search for specific files
                            fingerprints_names[name] = f"{library_path:}/{name:}.pdb"
            else:
...
def guess_fingerprint(
    ...
    if len(fp_files) == 0:
        logger.info(
            f"Not found templates with name including {metal_name:} with vdw type: {vdw_type:}"
        )
        return False
    ...
    if name_of_binding_side is None:
        logger.info(
            f"\t[-] Not found any fingerprints for {metal_name:} with vdw type: {vdw_type:}"
        )
        return False
