...
def neutralize_charge(file_name, output, charge=0):
    ...
    if abs(sum_of_charge) > 0.00001:
        ...
        qtot = 0.0
        logger.info(
            f"   [ ] Rounding up charges to make molecule neutral, every atom gets additional {-fract:}"
        )
        ...
        for line in moleculetype.splitlines():
            ...
            if in_atoms and len(line) > 0 and line[0] != ";" and len(line.split()) > 6:
                line = line[: line.find(";")]
                charge = float(line.split()[6]) - np.round(
                    fract
                    + np.sign(fract_rest)
                    * 0.000001
                    * (
                        atom_nr % every_nth == 0
                        and every_nth * int((np.abs(fract_rest) / 0.000001)) > atom_nr
                    ),
                    6,
                )
                qtot += charge  # w
                ...
                print(
                    "{:>6s}{:>11s}{:>7s}{:>8s}{:>6s}{:>7s}{:>10f}{:>11s} ; qtot {: 6f}".format(
                        *line.split()[:6], charge, line.split()[7], qtot
                    ),
                    file=File,
                )  # ,[str(np.round(float(line.split()[6])-fract,6))],line.split()[7]))                else:
                atom_nr += 1
            else:
