...
from metallicious.copy_topology_params import (
    ...
    adjust_dihedrals,
    adjust_impropers,
    adjust_pair_exclusions,
    update_pairs,
)
from metallicious.load_fingerprint import (
    find_mapping_of_fingerprint_on_metal_and_its_surroundings,
)
from metallicious.log import logger
from metallicious.utils import prepare_universe_from_stk
...
class patcher:
    ...
    def save(self, output_coords, output_topol, tmpdir_path):
        ...
        if output_coords.endswith(".gro"):
            ...
            if self.cage_coord.endswith(
                ".xyz"
            ):  # ParmED seems not to work with xyz files
        ...
        self.topol_new.write("temp_topol.top")

        if output_topol.endswith(".top"):
            shutil.copy("temp_topol.top", f"{output_topol:s}")
        else:
            topol = pmd.load_file("temp_topol.top")
            topol.save(f"{output_topol:s}", overwrite=True)

        os.remove("temp_topol.top")

    def copy_site_topology_to_supramolecular(
        self,
        sites,
        cage_coord=None,
        cage_topol=None,
        ...
        for site in sites:
            mapping_fp_to_new, _ = (
                find_mapping_of_fingerprint_on_metal_and_its_surroundings(
                    ...
                    site.metal_name,
                    site.fp_syst,
                    cutoff=site.ligand_cutoff,
                    covalent_cutoff=site.covalent_cutoff,
                    donors=site.donors,
                )
            )

            self.topol_new = adjust_charge(
                self.topol_new, site.fp_topol, mapping_fp_to_new
            )

            self.topol_new = adjust_bonds(
                self.topol_new, site.fp_topol, mapping_fp_to_new
            )

            self.topol_new = adjust_angles(
                self.topol_new, site.fp_topol, mapping_fp_to_new
            )

            self.topol_new = adjust_dihedrals(
                self.topol_new, site.fp_topol, mapping_fp_to_new
            )

            self.topol_new = adjust_impropers(
                self.topol_new, site.fp_topol, mapping_fp_to_new
            )

            self.topol_new = adjust_pair_exclusions(
                self.topol_new, site.fp_topol, mapping_fp_to_new
            )
...
class StkPatcher:
    ...
    def __init__(self, cage_molecule: stk.Molecule) -> None:
        self.path = None
        self.tmpdir_path = None
        self.cage = prepare_universe_from_stk(cage_molecule)
        self.topol_new = None  # this is our new topology
