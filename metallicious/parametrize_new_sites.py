...
from metallicious.asserts import (
    ...
    compare_topology_and_coords,
)
from metallicious.charges import calculate_charges2
from metallicious.copy_topology_params import (
    copy_angles,
    copy_bonds,
    copy_dihedrals,
    copy_impropers,
...
class supramolecular_structure:
    ...
    def prepare_initial_topology(
        ...
        if homoleptic_ligand_topol is not None:
            shutil.copyfile(
                homoleptic_ligand_topol, f"{subdir}/{homoleptic_ligand_topol}"
            )
        os.chdir(subdir)

        if method == "gaff" or homoleptic_ligand_topol is not None:
            metal_indicies = prepare_initial_topology(
                new_filename,
                self.metal_names,
                self.sites[0].metal_charge,
                coord_filename,
                topol_filename,
                self.vdw_type,
    ...
    def parametrize(
        ...
        if self.topol is None:
            raise Exception(
                "Topology file not specified, please provide topology, or use prepare_initial_topology=True"
            )
        ...
        parameter_copier.copy_site_topology_to_supramolecular(
            self.sites, cage_coord=self.filename, cage_topol=self.topol
        )

        parameter_copier.save(
            f"{self.path:s}/{out_coord:s}",
            f"{self.path:s}/{out_topol:s}",
...
class metal_site:
    ...
    def load_fingerprint(self):
        ...
        self.fp_topol, self.fp_syst = load_fp_from_file(
            self.fp_coord_file,
            self.fp_topol_file,
            self.fp_style,
            self.ignore_truncation_warning,
        )
        return True
    ...
    def set_cutoff(self):
        ...
        self.ligand_cutoff = (
            np.max(distance_array(metal_position, nometal_position)) + 2.0
        )
...
class new_metal_site:
    ...
    def partial_charge(self):
        ...
        partial_charges = calculate_charges2(
            self.metal_name,
            self.metal_charge,
            self.directory,
            self.unique_ligands_pattern,
            ...
            self.starting_index,
            metal_radius=self.metal_radius,
            mult=self.mult,
        )
        ...
        partial_charges = [
            partial_charge
            for idx, partial_charge in enumerate(partial_charges)
            if idx not in self.additional_atoms
        ]
    ...
    def reduce_to_template(self, out_topol="template.top", out_coord="template.pdb"):
        ...
        new_cage.atoms.write("old_new_template.pdb")

        logger.info(f"removing additional atoms {self.additional_atoms:}")

        if len(self.additional_atoms) > 0:
            template = new_cage.select_atoms(
                f"not index {' '.join(list(map(str, np.array(self.additional_atoms)))):s}"
            )
        else:
    ...
    def check_library(self):
        ...
        :return:
        """
        return guess_fingerprint(
            self.directory + "/saturated_template.xyz",
            0,
            metal_name=self.metal_name,
            metal_charge=self.metal_charge,
            vdw_type=self.vdw_type,
            rmsd_cutoff=self.rmsd_cutoff,
        )
