...
from metallicious.antechamber_interface import (
    antechamber,
    check_antechamber_if_available,
)
...
def mapping_itp_coords(syst, ligand_file):
    ...
    G_lig = nx.Graph(
        MDAnalysis.topology.guessers.guess_bonds(syst.atoms, syst.atoms.positions)
    )
    nx.set_node_attributes(
        G_lig, {atom.index: atom.name[0] for atom in syst.atoms}, "name"
    )
    topol = pmd.load_file(ligand_file)
    bonds = [(bond.atom1.idx, bond.atom2.idx) for bond in topol.bonds]
    G_top = nx.Graph(bonds)
    nx.set_node_attributes(
        G_top, {atom.idx: atom.name[0] for atom in topol.atoms}, "name"
    )
    iso = nx.isomorphism.GraphMatcher(
        G_lig, G_top, node_match=lambda n1, n2: n1["name"] == n2["name"]
    )
    ...
    for z, nodes in enumerate(nx.connected_components(G1)):
        ...
        for top_idx, Gtop in enumerate(Gtops):
            ...
            if iso.is_isomorphic():
                ...
                logger.debug(
                    "cannot match the ligands. No need to worry if heteroleptic. Otherwise you need to create topology with nonboned metal manually (or you might try to optimize structure"
                )
...
def get_args():
    ...
    parser.add_argument(
        "-output_coord",
        default="cage.gro",
        help="Output topology structure of the cage",
    )
    parser.add_argument(
        "-output_top", default="cage.top", help="Output topology structure of the cage"
    )
    parser.add_argument(
        "-metal_vdw", default="merz-tip3p", help="Where to take parameters from"
    )
