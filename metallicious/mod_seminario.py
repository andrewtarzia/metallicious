...
def modified_seminario_method(
    hessian, coords, atom_names, bond_list, angle_list, vibrational_scaling=1
):
    N = len(coords)
    ...
    bonds = bonds_calculated_printed(
    ...
    angles = angles_calculated_printed(
        vibrational_scaling_squared,
        angle_list,
        bond_lengths,
        atom_names,
...
def bonds_calculated_printed(
    ...
    for i in range(0, len(bond_list)):
        ...
        fid.write(
            atom_names[bond_list[i][0]] + "-" + atom_names[bond_list[i][1]] + "  "
        )
        fid.write(
            str("%#.5g" % k_b[i])
            + "   "
        ...
        sorted_atom_name = (
            atom_names[bond_list[i][order[0]]],
            atom_names[bond_list[i][order[1]]],
        )
...
def angles_calculated_printed(
    ...
    for i in range(0, len(central_atoms_angles)):
        ...
        for j in range(0, len(central_atoms_angles[i])):
            ...
            while ((j + n) < len(central_atoms_angles[i])) and central_atoms_angles[i][
                j
            ][0] == central_atoms_angles[i][j + n][0]:
    ...
    for i in range(0, len(angle_list)):
        ...
        ):  # we have indecies from smaler to larger in the angles
            order = [2, 1, 0]

        sorted_angle_list = (
            angle_list[i][order[0]],
            angle_list[i][order[1]],
            angle_list[i][order[2]],
        )
        sorted_atom_name = (
            atom_names[angle_list[i][order[0]]],
            atom_names[angle_list[i][order[1]]],
            atom_names[angle_list[i][order[2]]],
        )
