...
def find_metal_indices(cage, metal_name):
    ...
    if not hasattr(cage.atoms[0], "element"):
        guessed_elements = MDAnalysis.topology.guessers.guess_types(
            strip_numbers_from_atom_names(cage.atoms.names)
        )
        cage.universe.add_TopologyAttr("elements", guessed_elements)

    metal_indices = [
        idx
        for idx, element in enumerate(cage.atoms.elements)
        if element.title() == metal_name.title()
    ]
    n_metals = len(metal_indices)

    if (
        n_metals == 0
    ):  # this happens when guesser fails, for example Pd becomes P. We need to try using just names
        metal_indices = [
            idx
            for idx, name in enumerate(cage.atoms.names)
            if name[: len(metal_name)].title() == metal_name.title()
        ]
        n_metals = len(metal_indices)
...
def find_bound_ligands_nx(
    cage,
    metal_index,
    cutoff=7,
    cutoff_covalent=3.0,
    closest_neighbhors=3,
    neighbhor_cutoff=None,
    donors=None,
):
    """
    ...
    if sum([len(closest_atom) == 0 for closest_atom in closest_atoms_ligands]) != 0:
        raise ValueError(
            f"Something's not right. One of the ligands not connected to metal. Are donors selected correctly? (currently: {donors:}"
        )
    ...
    if neighbhor_cutoff is not None:
        for idx, closest_atoms in enumerate(closest_atoms_ligands):
            ...
            for closest_atom in closest_atoms:
                ...
                new_nodes = nx.generators.ego_graph(
                    G_sub_cages_bound[idx], closest_atom, radius=neighbhor_cutoff - 1
                ).nodes
                nodes += list(new_nodes)
            nodes = list(set(nodes))

            for G_sub_cage in nx.connected_components(
                nx.subgraph(G_sub_cages_bound[idx], nodes)
            ):
                ...
                G_sub_cages_bound_cutoff.append(
                    G_sub_cages_bound[idx].subgraph(G_sub_cage)
                )
...
def find_closest_and_add_rings(
    metal_index, cage, bound_ligands, selected_closest_atoms, aromaticity
):
    """
    ...
    for idx_bound_ligand, bound_ligand in enumerate(bound_ligands):
            ...
            if closest_atom in selected_bound_ligand.nodes:
                neighbour_indices += list(
                    nx.generators.ego_graph(
                        selected_bound_ligand, closest_atom, radius=2
                    ).nodes
                )
                direct_neighbour_indices += list(
                    nx.generators.ego_graph(
                        selected_bound_ligand, closest_atom, radius=1
                    ).nodes
                )
        ...
        def recursive_rings(ring_of_intrest, rings, aromaticity):
            ...
            for ring in rings:
                ...
                if (
                    len(np.intersect1d(ring_of_intrest, ring)) >= 2
                    and len(set(ring_of_intrest + ring)) > len(ring_of_intrest)
                    and len(set(ring_of_intrest + ring)) > len(ring)
                ):
        ...
        close_atoms_and_in_rings = list(
            set(atoms_within_cutoff + rings_with_closest_atom)
        )
        ...
        close_atoms_and_in_rings = list(
            np.concatenate(
                [
                    list(Gsub_connect)
                    for Gsub_connect in nx.connected_components(Gsub)
                    if len(Gsub_connect) > 1
                ]
            )
        )
        ...
        for edge in selected_bound_ligand.edges:
            if (
                edge[0] in close_atoms_and_in_rings
                and edge[1] not in close_atoms_and_in_rings
            ):
                adj_atoms.append(edge[1])
            elif (
                edge[1] in close_atoms_and_in_rings
                and edge[0] not in close_atoms_and_in_rings
            ):
        ...
        site_link_atoms_single += [
            node
            for node in dict(degrees_subset)
            if degrees_subset[node] != degrees_full[node]
        ]
...
def find_closest_and_add_rings_iterate(
    metal_indices,
    cage,
    all_metal_indecies,
    covalent_cutoff=3.0,
    ...
    for metal_index in all_metal_indecies:
        cage.atoms[
            metal_index
        ].type = "H"  # we change type of metal for hydrogen, becasue vdwradii is required for aromaticity

    if not hasattr(cage.atoms[0], "element"):
        guessed_elements = MDAnalysis.topology.guessers.guess_types(
            strip_numbers_from_atom_names(cage.names)
        )
    ...
    for metal_index in metal_indices:
        ...
        bound_ligands, selected_closest_atoms = find_bound_ligands_nx(
            cage,
            metal_index,
            cutoff=None,
            cutoff_covalent=covalent_cutoff,
            donors=donors,
            closest_neighbhors=closest_neighbhors,
        )

        site_single, _, site_link_atoms_single = find_closest_and_add_rings(
            metal_index, cage, bound_ligands, selected_closest_atoms, aromaticity
        )
        site += site_single
        ...
        binding_sites_graphs.append(
            [metal_index] + list(sorted(set(site)))
        )  # The sorting is required for the later checking of uniqueness (making ordering of atoms in figerprint hassle-free)
...
def check_uniqueness(
    binding_sites_graphs, site_link_atoms, structure, metal_name, rmsd_cutoff=2
):
    ...
    logger.info(
        f"Uniqueness check: Number of binding sites to check: {len(binding_sites_graphs):}"
    )
    ...
    for site, site_link_atom in zip(binding_sites_graphs, site_link_atoms):
        ...
        for site_fingerprint in unique_sites:
            ...
            if len(structure[site]) == len(structure[site_fingerprint]):
                _, rmsd = map_two_structures(
                    0, structure[site], structure[site_fingerprint], metal_name
                )
            else:
                # print("NOT The same")
                rmsd = 1e10
            ...
            logger.info(
                f"RMSD between current structure and already check structures: {rmsd:}"
            )
        ...
        if exist == False:
            logger.info(
                f"Adding unique site # {len(unique_sites):}, {[len(a) for a in unique_sites]:}"
            )
...
def rotation_matrix_from_vectors(vector1, vector2, theta):
    ...
    """
    a, b = (
        (vector1 / np.linalg.norm(vector1)).reshape(3),
    ...
    th = np.deg2rad(theta)
    rotation_matrix_around_vector2 = (
        np.cos(th) * np.eye(3)
        + np.sin(th) * kmat2
        + (1 - np.cos(th)) * np.outer(v1, v1)
    )
...
def add_hydrogens(selection_site, metal_name, add_atoms_to_this_atom):
    ...
    methane = np.array(
        [
            [-0.00000935761277, -0.00000001492783, 0.00002482854051],
    ...
    for link_atom in add_atoms_to_this_atom:
        if selection_site.universe.dimensions is not None:
            G_selection_site = nx.Graph(
                MDAnalysis.topology.guessers.guess_bonds(
                    selection_site.atoms,
        ...
        if len(positions_of_additional_hydrogens) == 0:
            rot = rotation_matrix_from_vectors(
                molecule[1] - molecule[0], bond_vector, 0
            )
            newpos_final = rot.dot(molecule.T).T[2:]
            ...
            for theta in range(0, 360, 12):
                rot = rotation_matrix_from_vectors(
                    molecule[1] - molecule[0], bond_vector, theta
                )
                newpos = rot.dot(molecule.T).T[2:]
                dist = np.min(
                    distance_array(
                        np.concatenate(positions_of_additional_hydrogens), newpos
                    )
                )
                if dist > min_dist:
                    min_dist = dist
    ...
    renumered_add_atoms_to_this_atom = [
        idx
        for idx, name in enumerate(selection_site.atoms.indices)
        if name in add_atoms_to_this_atom
    ]
...
def find_atom_to_ligand_membership(new_syst, metal_name):
    ...
    if new_site_no_metal.universe.dimensions is not None:
        G_all_short_ligands = nx.Graph(
            MDAnalysis.topology.guessers.guess_bonds(
                new_site_no_metal.atoms,
                new_site_no_metal.atoms.positions,
        ...
        G_all_short_ligands = nx.Graph(
            MDAnalysis.topology.guessers.guess_bonds(
                new_site_no_metal.atoms, new_site_no_metal.atoms.positions
            )
        )

    nx.set_node_attributes(
        G_all_short_ligands,
        {atom.index: atom.name[0] for atom in new_site_no_metal.atoms},
        "name",
    )
    G_sub_short_ligands = [
        G_all_short_ligands.subgraph(a)
        for a in nx.connected_components(G_all_short_ligands)
    ]
    ...
    ligands_atoms_membership = [
        list(G_sub_short_ligands[a].nodes) for a in range(len(G_sub_short_ligands))
    ]
...
def find_ligand_pattern(new_syst, ligands_nodes):
    ...
    if not hasattr(new_syst.atoms[0], "element"):
        guessed_elements = guess_types(
            strip_numbers_from_atom_names(new_syst.atoms.names)
        )
    ...
    for ligands_node in ligands_nodes:
        ...
        mol1 = selected_ligand_1.select_atoms("not element DUMMY").convert_to(
            "RDKIT", force=True
        )

        for idx, unique_lingad in enumerate(unique_ligands):
            selected_ligand_2 = MDAnalysis.Merge(new_syst.atoms[unique_lingad])
            mol2 = selected_ligand_2.select_atoms("not element DUMMY").convert_to(
                "RDKIT", force=True
            )
...
def renumer_ligands(
    ...
    for idx, ligands_node in enumerate(ligands_atoms_membership):
        sorted_extra_atoms = [
            idx for idx, a in enumerate(ligands_node) if a in extra_atoms
        ]
        sorted_link_atoms = [
            idx for idx, a in enumerate(ligands_node) if a in link_atoms
        ]
        ...
        G_ligand_1 = nx.Graph(
            MDAnalysis.topology.guessers.guess_bonds(
                selected_ligand_1.atoms,
                selected_ligand_1.atoms.positions,
        ...
        nx.set_node_attributes(
            G_ligand_1,
            {atom.index: atom.name[0] for atom in selected_ligand_1.atoms},
            "name",
        )
        nx.set_node_attributes(
            G_ligand_1,
            {chirality for chirality in guess_chirality(selected_ligand_1.atoms)},
            "chirality",
        )

        # selected_ligand_2 = MDAnalysis.Merge(new_syst.atoms[unique_ligands[unique_ligands_pattern[idx]]] + metals) # TODO
        selected_ligand_2 = MDAnalysis.Merge(
            new_syst.atoms[unique_ligands[unique_ligands_pattern[idx]]]
        )

        G_ligand_2 = nx.Graph(
            MDAnalysis.topology.guessers.guess_bonds(
                selected_ligand_2.atoms,
                selected_ligand_2.atoms.positions,
                vdwradii={metal_name: covalent_cutoff},
            )
        )
        nx.set_node_attributes(
            G_ligand_2,
            {atom.index: atom.name[0] for atom in selected_ligand_2.atoms},
            "name",
        )
        nx.set_node_attributes(
            G_ligand_2,
            {chirality for chirality in guess_chirality(selected_ligand_2.atoms)},
            "chirality",
        )
        ...
        iso = isomorphism.GraphMatcher(
            G_ligand_1, G_ligand_2, node_match=lambda n1, n2: n1["name"] == n2["name"]
        )
        ...
        if iso.is_isomorphic():
            for node in iso.mapping:
                new_ligand.atoms[iso.mapping[node]].position = selected_ligand_1.atoms[
                    node
                ].position
...
def read_and_reoder_topol_and_coord(
    filename, topol_filename, metal_name, all_metal_names=None
):
    ...
    if all_metal_names is not None:
        for additional_metal in all_metal_names:
            ...
            if hasattr(syst.atoms, "elements"):
                syst.atoms[additional_indices].elements = [additional_metal] * len(
                    additional_indices
                )
    else:
    ...
    topol_nonmetal.strip(
        f"@{','.join(list(map(str, np.array(all_metals_indices) + 1))):s}"
    )
...
def extract_metal_structure(
    filename,
    topol_filename,
    metal_name,
    output=None,
    check_uniquness=True,
    all_metal_names=None,
    covalent_cutoff=3.0,
    donors=None,
    closest_neighbhors=3,
):
    ...
    cage, topol, all_metals_indices = read_and_reoder_topol_and_coord(
        filename, topol_filename, metal_name, all_metal_names
    )
    ...
    all_ligands_atoms = cage.select_atoms(
        f"not index {' '.join(map(str, all_metals_indices)):}"
    )

    G_all_ligands = nx.Graph(
        MDAnalysis.topology.guessers.guess_bonds(
            all_ligands_atoms.atoms,
            all_ligands_atoms.atoms.positions,
    ...
    nx.set_node_attributes(
        G_all_ligands,
        {atom.index: atom.name[0] for atom in all_ligands_atoms.atoms},
        "name",
    )

    binding_sites_graphs, site_link_atoms = find_closest_and_add_rings_iterate(
        metal_indices,
        cage,
        all_metals_indices,
        covalent_cutoff,
    ...
    if check_uniquness:
        unique_sites, unique_site_link_atoms = check_uniqueness(
            binding_sites_graphs, site_link_atoms, cage, metal_name
        )
    else:
    ...
    for n_site, (unique_site, metal_index) in enumerate(
        zip(unique_sites, metal_indices)
    ):
        ...
        if selection_site.universe.dimensions is not None:
            selection_site.atoms[
                0
            ].type = metal_name  # previously, we changed it to H (aromaticity) we change it back
            selection_site = unwrap(selection_site, metal_name)

        new_syst, new_link_atoms = add_hydrogens(
            selection_site, metal_name, unique_site_link_atoms[n_site]
        )
        ...
        unique_ligands_pattern, unique_ligands = find_ligand_pattern(
            new_syst, ligands_atoms_membership
        )
        ...
        for idx, unique_ligand in enumerate(unique_ligands):
            ...
            selected_atoms_indices = [
                idx
                for idx in new_syst.atoms[: len(selection_site)].indices
                if idx in unique_ligand
            ]

            ligand_topol.strip(
                f"!@{','.join(list(map(str, np.array(selected_atoms_indices) + 1))):s}"
            )
            ...
            mol = ligand_coord.select_atoms("not element DUMMY").convert_to(
                "RDKIT", force=True
            )
        ...
        renumbered_ligands, categorized_extra_atoms, categorized_link_atoms = (
            renumer_ligands(
                new_syst,
                metal_name,
                ligands_atoms_membership,
        ...
        sorted_renumbered_ligands = [
            renumbered_ligands[a]
            for a in np.append(0, np.argsort(unique_ligands_pattern) + 1)
        ]

        new_cage = MDAnalysis.Merge(
            *sorted_renumbered_ligands
        )  # , dimensions=crystal.dimensions)
        ...
        indecies = [
            list(range(starting_index[a - 1], starting_index[a]))
            for a in range(1, len(starting_index))
        ]

        charge_pattern = [
            unique_ligands_charges[unique_ligands_pattern[a]]
            for a in np.argsort(unique_ligands_pattern)
        ]
        smiles_pattern = [
            unique_ligands_smiles[unique_ligands_pattern[a]]
            for a in np.argsort(unique_ligands_pattern)
        ]
        sorted_renumbered_extra_atoms = [
            np.array(categorized_extra_atoms[a], dtype=int)
            for a in np.argsort(unique_ligands_pattern)
        ]
        sorted_renumbered_extra_atoms = list(
            np.concatenate(
                [
                    starting_index[idx + 1] + extra_atoms
                    for idx, extra_atoms in enumerate(sorted_renumbered_extra_atoms)
                ]
            )
        )

        sorted_renumbered_extra_link_atoms = [
            np.array(categorized_link_atoms[a], dtype=int)
            for a in np.argsort(unique_ligands_pattern)
        ]
        sorted_renumbered_extra_link_atoms = list(
            np.concatenate(
                [
                    starting_index[idx + 1] + link_atoms
                    for idx, link_atoms in enumerate(sorted_renumbered_extra_link_atoms)
                ]
            )
        )

        selected_metal_site_list = [
            metal_name,
            0,
            1,
            f"{output:s}{n_site:d}",
            unique_ligand_filenames,
            np.sort(unique_ligands_pattern),
            sorted_renumbered_extra_link_atoms,
            sorted_renumbered_extra_atoms,
            starting_index,
            indecies,
